...
template <typename ETX = double> class SyncManager {
 public:
    ...
    void removeRow(const ETX& indRow);

    // Remove row by index in table
    void removeRowAtIndex(const size_t& i);

    // Get read only reference to data table
    const OpenSim::DataTable& getTable();
...
template <typename ETX>
SyncManager<ETX>::SyncManager(const ETX& samplingRate, const ETX& threshold) {
    _entryThreshold = threshold;
    _samplingRate = samplingRate;
}

template <typename ETX>
std::pair<ETX, std::vector<SimTK::Vector_<ETX>>>
SyncManager<ETX>::getPack(size_t delay) {
    ...
    if (isNewRowFinite && (t2 <= v.back()) && !SimTK::isInf(t)) {
        ...
        for (size_t i = 0; i < _numOfPacks; ++i) {
            pack.push_back(~_table.getRow(t)(columnId, _vectorSizePerPack[i]));
...
template <typename ETX>
template <typename... Args>
void SyncManager<ETX>::appendPack(Args&&... args) {
    ...
    if (!_tableSizeIsSet) {
        ...
        map<0, Args...>(argTuple, [&](auto&& p,
            ...
            static_assert(is_pair<std::remove_cv_t<
            ...
            if constexpr (is_simtk_vec<pairSecondElement_t>::value ||
                ...
                throw std::invalid_argument("Invalid std::pair::second type");
            }
        });
    ...
    map<0, Args...>(argTuple, [&](auto&& p,
        ...
        typedef std::remove_cv_t<std::remove_reference_t<decltype(p.second)>>
                pairSecondElement_t;
        ETX indRow;
        ...
        if (isUniqueEntry(p.first, _entryThreshold, indRow)) { // append
            ...
            if constexpr (is_simtk_vec<pairSecondElement_t>::value ||
                ...
                throw std::invalid_argument("SyncManager: invalid argument");
            }
            ...
            if constexpr (is_simtk_vec<pairSecondElement_t>::value ||
                ...
                for (int i = 0; i < p.second.size(); ++i) {
                    ...
                    for (int j = 0; j < v.size(); ++j) {
                        _table.updRow(indRow).updCol(j + columnId + offset) =
                                std::move(v[j]);
                    }
                    offset += v.size();
                }
...
template <typename ETX>
template <typename... Args>
void SyncManager<ETX>::appendRow(const ETX& indRow, Args&&... args) {
    _table.appendRow(indRow, std::forward<Args>(args)...);
}

template <typename ETX>
template <typename... Args>
void SyncManager<ETX>::setRow(const ETX& indRow, Args&&... args) {
    _table.setRow(indRow, std::forward<Args>(args)...);
}

template <typename ETX>
template <typename... Args>
void SyncManager<ETX>::setRowAtIndex(const size_t& index, Args&&... args) {
    _table.setRowAtIndex(index, std::forward<Args>(args)...);
}

template <typename ETX>
template <typename... Args>
void SyncManager<ETX>::updRowAtIndex(const size_t& index, Args&&... args) {
    _table.updRow(index, std::forward<Args>(args)...);
}

template <typename ETX> void SyncManager<ETX>::removeRow(const ETX& indRow) {
    _table.removeRow(indRow);
}

template <typename ETX>
void SyncManager<ETX>::removeRowAtIndex(const size_t& i) {
    _table.removeRowAtIndex(i);
}

template <typename ETX> const OpenSim::DataTable& SyncManager<ETX>::getTable() {
    return _table;
}
...
inline typename std::enable_if<I == sizeof...(Args), void>::type
SyncManager<ETX>::map(std::tuple<Args...>& t, F&& f) { // terminate call
    _offset = 0;
    _packCounter = 0; // WARNING value is renewed for every appendPack().  Not
...
template <typename ETX>
        ...
        SyncManager<ETX>::map(std::tuple<Args...>& t, F&& f) {
    ...
    if constexpr (is_container<std::remove_cv_t<
...
template <typename ETX>
bool SyncManager<ETX>::isUniqueEntry(const double& value, const ETX& th,
                                     ETX& indRow) {
    const auto& v = _table.getIndependentColumn();
...
template <typename ETX> void SyncManager<ETX>::reorderTable() {
    const auto& v = _table.getIndependentColumn();
...
template <typename ETX>
SimTK::Vector SyncManager<ETX>::createRow(const ETX& t) {
...
template <typename ETX>
template <typename V, typename F>
std::vector<size_t> SyncManager<ETX>::find_if_indexesInVector(V&& vec,
                                                              F&& function) {
...
template <typename ETX> void SyncManager<ETX>::interpolateNanValues() {
    for (size_t col = 0; col < _table.getNumColumns(); ++col) {
...
template <typename ETX>
SimTK::Vector SyncManager<ETX>::interpolate(const SimTK::Vector& x,
    ...
    OpenSim::PiecewiseLinearFunction linearFun((int) x.size(), &x[0], &y[0]);
...
template <typename ETX>
template <typename InputIterator>
void SyncManager<ETX>::deleteRows(InputIterator first, InputIterator last) {
