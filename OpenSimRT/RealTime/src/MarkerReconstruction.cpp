...
bool MarkerReconstruction::isValidFrame(const InverseKinematics::Input& frame) {
    // check for missing markers
    for (unsigned int i = 0; i < frame.markerObservations.size(); i++) {
        if (frame.markerObservations[i] == Vec3(0, 0, 0) ||
            !frame.markerObservations[i].isFinite()) {
            // cout << "Marker with observation ID = " << i
            //      << " went missing at t = " << frame.t << endl;
            return false;
        }
    }
    return true;
...
/**
 * @brief Reconstruct possible missing markers based on previous valid frame and
 * two closest markers in current frame. The basic notion behind this method is
 * that the distance between close markers does not change between frames.
 *
 ...
 *
 * @param [currentObservations]: reference to an array containing the positions
 * of the markers in the current frame.
 */
...
/**
 * @brief Find intersecting points of two spheres.
 *
 * Modified version of:
 * http://paulbourke.net/geometry/circlesphere/
 * written by Paul Bourke, 1997.
 *
 * @param c1 - First sphere
 * @param c2 - Second sphere
 *
 * Returns a point to Circle object. If the two spheres do not intersect the
 * pointer is null.
 */
...
/**
 * @brief Given a point x in space, find the closest point in the circumference
 * of a given circle lying in the 3D space.
 *
 * @param vec - Input point x
 * @param c - Circle object (origin, radius, normal)
 * @return Vec3 - Output point
 */
...
/**
 * @brief Find the closests N markers to the missing marker.
 *
 * @param [mMarkerName]: name of the missing marker
 * @param [currentObservations] - Array with positions of the markers in current
 * frame.
...
MarkerReconstruction::initializeLogger() {
