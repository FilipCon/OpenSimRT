...
OpenSim::TimeSeriesTable MuscleOptimization::initializeMuscleLogger() {
...
void OptimizationTarget::build() {
    ...
    for (int i = 0; i < na; ++i) {
        ...
        if (muscle) {
            ...
            lowerBounds[i] = coordAct->getMinControl() * xMax[i];
            upperBounds[i] = coordAct->getMaxControl() * xMax[i];
        } else {
