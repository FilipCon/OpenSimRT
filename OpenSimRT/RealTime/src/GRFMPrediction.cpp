...
GRFMPrediction::GRFMPrediction(const Model& otherModel,
    ...
    //     auto k1 = std::exp(4.0 / 9.0);
    //     auto k2 = k1 * std::exp(-16.0 / 9.0) / 2.0;
    //     return clip(k1 * std::exp(-std::pow(2.0 * (t - Tds / 3.0) / Tds, 2))
    //     -
    //                         2.0 * k2 * t / Tds,
    //                 0.0, SimTK::Infinity);
    // };
    ...
    // anteriorForceTransition = [&](const double& t) -> double {
    //     double A = -0.2138465309835869;
    //     double K = 3.1630589985381152;
    //     double B = 28.2381496577766;
    //     double M = -1.9066072702207824;
    //     double m1 = 0.5229664449407797;
    //     double m2 = -0.1121957796012321;
    //     double c = -0.07245652389053765;
    //     return A + K / (1.0 + std::exp(B * (t - m1 * Tds))) +
    //            M * std::exp(-std::pow((t - m2 * Tds), 2) /
    //                         (2 * std::pow(c, 2)));
    // };
