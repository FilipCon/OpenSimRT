...
void run() {
    ...
    for (int i = 0; i < qTable.getNumRows(); i++) {
        ...
        grfRightWrench.point =
                model.getGround().findStationLocationInAnotherFrame(
                        state, grfRightWrench.point,
                        model.getBodySet().get("calcn_r"));
        grfLeftWrench.point =
                model.getGround().findStationLocationInAnotherFrame(
                        state, grfLeftWrench.point,
                        model.getBodySet().get("calcn_l"));

        // log data (use filter time to align with delay)
        grfRightLogger.appendRow(t, ~grfRightWrench.toVector());
        grfLeftLogger.appendRow(t, ~grfLeftWrench.toVector());
    }

    // store results
    STOFileAdapter::write(grfRightLogger,
                          subjectDir +
                                  "experimental_data/wrench_right_in_foot.sto");
    STOFileAdapter::write(grfLeftLogger,
                          subjectDir +
                                  "experimental_data/wrench_left_in_foot.sto");
}
